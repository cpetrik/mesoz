...
o1 <- ggplot(subset(NM,biome==1), aes(y=mesoz, x=chl)) + theme_bw(base_size=12) +  
  geom_point() + geom_smooth(method="lm") +  
  ylab("obs mesoz") + xlab("chl") + #ggtitle("LC") +
  scale_y_log10() + scale_x_log10() 
o2 <- ggplot(subset(NM,biome==2), aes(y=mesoz, x=chl)) + theme_bw(base_size=12) +  
  geom_point() + geom_smooth(method="lm") +  
  ylab("") + xlab("chl") + #ggtitle("HCSS") +
  scale_y_log10() + scale_x_log10() 
o3 <- ggplot(subset(NM,biome==3), aes(y=mesoz, x=chl)) + theme_bw(base_size=12) +  
  geom_point() + geom_smooth(method="lm") +  
...
write.table(Cff1,"Hist_coeffs_mod_chl_biome1_LC_obsglm_LC45.csv",sep=",",row.names=T)
