...
diff_cz = cell_area.*(log10(FCzm*1e3) - log10(HCzm*1e3));
diff_mz = cell_area.*(log10(FMzm*1e3) - log10(HMzm*1e3));
diff_nz = cell_area.*(log10(FNzm*1e3) - log10(HNzm*1e3));
diff_gz = cell_area.*(log10(FGzm*1e3) - log10(HGzm*1e3));
diff_iz = cell_area.*(log10(FIzm*1e3) - log10(HIzm*1e3));
diff_uz = cell_area.*(log10(FUzm*1e3) - log10(HUzm*1e3));
...
Tmeanz = array2table(zmeans_diff,'RowNames',simtext,'VariableNames',...
    {'Global','LC','HCSS','HCPS'});
writetable(Tmeanz,[sfile 'delta_log10_means_areaw_hist_ssp585_50yr_zmeso200_histbiomes_mgC.csv'],'WriteRowNames',true);
save('delta_log10_means_areaw_hist_ssp585_50yr_zmeso200_histbiomes_mgC.mat',...
    'Tmeanz','zmeans_diff');
