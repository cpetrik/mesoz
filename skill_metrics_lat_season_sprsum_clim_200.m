...
%% 
load('obs_mod_chl_ntemp_sprsum_clim_200.mat')
obs = obsmodchl(:,3);
model = obsmodchl(:,4:8);
...
subplot(3,2,2)
bar(skill(1,:),'k')
text(-1,0.35,'Correlation coefficient','FontWeight','Bold','HorizontalAlignment','center')
set(gca,'XTickLabel',simtex)
ylim([0 0.3])

subplot(3,2,4)
bar(skill(2,:),'k')
text(-1,0.95,'Root mean square error','FontWeight','Bold','HorizontalAlignment','center')
set(gca,'XTickLabel',simtex)
ylim([0 0.8])

subplot(3,2,6)
bar(skill(6,:),'k')
text(-1,0.2,'Modeling efficiency','FontWeight','Bold','HorizontalAlignment','center')
set(gca,'XTickLabel',simtex)
ylim([-1.1 0.1])
stamp('N Temp Spr-Sum')
print('-dpng','corr_rmse_mef_ntemp_sprsum_clim_200.png')
...
print('-dpng','Taylor_ntemp_sprsum_clim_200')
