...
# load data
CM <- read.csv("can_hist_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
NM <- read.csv("cnrm_hist_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
GM <- read.csv("gfdl_hist_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
IM <- read.csv("ipsl_hist_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
UM <- read.csv("ukesm_hist_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
...
## Latitude > - 60 N & <= -30 N: SON + DJF 
aCM <- subset(CM, Lat > -60 & Lat < -30)
aNM <- subset(NM, Lat > -60 & Lat < -30)
aGM <- subset(GM, Lat > -60 & Lat < -30)
aIM <- subset(IM, Lat > -60 & Lat < -30)
aUM <- subset(UM, Lat > -60 & Lat < -30)
Amz <- subset(Ann, Lat > -60 & Lat < -30)

## Take mean over SON + DJF
aCM$chl <- (aCM$chlSON+aCM$chlDJF)/2
aNM$chl <- (aNM$chlSON+aNM$chlDJF)/2
aGM$chl <- (aGM$chlSON+aGM$chlDJF)/2
aIM$chl <- (aIM$chlSON+aIM$chlDJF)/2
aUM$chl <- (aUM$chlSON+aUM$chlDJF)/2

aCM$sst <- (aCM$sstSON+aCM$sstDJF)/2
aNM$sst <- (aNM$sstSON+aNM$sstDJF)/2
aGM$sst <- (aGM$sstSON+aGM$sstDJF)/2
aIM$sst <- (aIM$sstSON+aIM$sstDJF)/2
aUM$sst <- (aUM$sstSON+aUM$sstDJF)/2

aCM$zoo <- (aCM$zooSON+aCM$zooDJF)/2
aNM$zoo <- (aNM$zooSON+aNM$zooDJF)/2
aGM$zoo <- (aGM$zooSON+aGM$zooDJF)/2
aIM$zoo <- (aIM$zooSON+aIM$zooDJF)/2
aUM$zoo <- (aUM$zooSON+aUM$zooDJF)/2
...
a1 <- ggplot(aCM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  ...
  scale_y_log10() + scale_x_log10() + 
   annotate( geom = 'text', y = 1e3, x = 2e-5, hjust = 0, label=paste0("r = ",signif(Ccorr_all_r[2,1],digits = 2)), size=5) +
   annotate( geom = 'text', y = 1e2, x = 2e-5, hjust = 0, label=paste0("p = ",signif(Ccorr_all_p[2,1],digits = 2)), size=5)

a2 <- ggplot(aNM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("CNRM zmeso mgC m-2") + xlab("CNRM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 3e3, x = 2e-5, hjust = 0, label=paste0("r = ",signif(Ncorr_all_r[2,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 2e3, x = 2e-5, hjust = 0, label=paste0("p = ",signif(Ncorr_all_p[2,1],digits = 2)), size=5)

a3 <- ggplot(aGM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("GFDL zmeso mgC m-2") + xlab("GFDL chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 3e3, x = 2e-5, hjust = 0, label=paste0("r = ",signif(Gcorr_all_r[2,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 2e3, x = 2e-5, hjust = 0, label=paste0("p = ",signif(Gcorr_all_p[2,1],digits = 2)), size=5)

a4 <- ggplot(aIM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("IPSL zmeso mgC m-2") + xlab("IPSL chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 1.5e3, x = 2e-5, hjust = 0, label=paste0("r = ",signif(Icorr_all_r[2,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 1e3, x = 2e-5, hjust = 0, label=paste0("p = ",signif(Icorr_all_p[2,1],digits = 2)), size=5)

a5 <- ggplot(aUM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("UKESM zmeso mgC m-2") + xlab("UKESM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 3e3, x = 2e-5, hjust = 0, label=paste0("r = ",signif(Ucorr_all_r[2,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 2e3, x = 2e-5, hjust = 0, label=paste0("p = ",signif(Ucorr_all_p[2,1],digits = 2)), size=5)
...
pdf( file = 'corr_mesoz_chl_Stemp_FalWin_clim_200_log.pdf', width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
s1 <- ggplot(aCM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  ...
  annotate( geom = 'text', y = 600, x = 0, hjust = 0, label=paste0("r = ",signif(Ccorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 550, x = 0, hjust = 0, label=paste0("p = ",signif(Ccorr_all_p[3,1],digits = 2)), size=5)

s2 <- ggplot(aNM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("CNRM zmeso mgC m-2") + xlab("CNRM sst") + 
  annotate( geom = 'text', y = 3000, x = 0, hjust = 0, label=paste0("r = ",signif(Ncorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 2500, x = 0, hjust = 0, label=paste0("p = ",signif(Ncorr_all_p[3,1],digits = 2)), size=5)
...
s4 <- ggplot(aIM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  ...
  annotate( geom = 'text', y = 2000, x = 0, hjust = 0, label=paste0("r = ",signif(Icorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 1800, x = 0, hjust = 0, label=paste0("p = ",signif(Icorr_all_p[3,1],digits = 2)), size=5)

s5 <- ggplot(aUM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("UKESM zmeso mgC m-2") + xlab("UKESM sst") + 
  annotate( geom = 'text', y = 5000, x = 0, hjust = 0, label=paste0("r = ",signif(Ucorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 4500, x = 0, hjust = 0, label=paste0("p = ",signif(Ucorr_all_p[3,1],digits = 2)), size=5)

pdf( file = 'corr_mesoz_sst_Stemp_FalWin_clim_200_log.pdf', width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
write.table(Cff,"Hist_coeffs_mod_chl_Stemp_FalWin_200.csv",sep=",",row.names=T)
...
write.table(Sff,"Hist_coeffs_mod_sst_Stemp_FalWin_200.csv",sep=",",row.names=T)
...
# load data
CM <- read.csv("can_ssp585_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
NM <- read.csv("cnrm_ssp585_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
GM <- read.csv("gfdl_ssp585_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
IM <- read.csv("ipsl_ssp585_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
UM <- read.csv("ukesm_ssp585_mod_zoo_chl_sst_FalWin_clim_200.csv",sep=",",header = T,stringsAsFactors = F)
...
a2 <- ggplot(aNM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  ...
  annotate( geom = 'text', y = 1.5e3, x = 3e-5, hjust = 0, label=paste0("p = ",signif(Ncorr_all_p[2,1],digits = 2)), size=5)
...
a5 <- ggplot(aUM, aes(y=zoo, x=chl)) + theme_bw(base_size=14) +  
  ...
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 5e3, x = 2e-6, hjust = 0, label=paste0("r = ",signif(Ucorr_all_r[2,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 3e3, x = 2e-6, hjust = 0, label=paste0("p = ",signif(Ucorr_all_p[2,1],digits = 2)), size=5)

pdf( file = 'corr_ssp585_mesoz_chl_Stemp_FalWin_clim_200_log.pdf', width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
s2 <- ggplot(aNM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("CNRM zmeso mgC m-2") + xlab("CNRM sst") + 
  annotate( geom = 'text', y = 2500, x = 0, hjust = 0, label=paste0("r = ",signif(Ncorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 2300, x = 0, hjust = 0, label=paste0("p = ",signif(Ncorr_all_p[3,1],digits = 2)), size=5)

s3 <- ggplot(aGM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("GFDL zmeso mgC m-2") + xlab("GFDL sst") + 
  annotate( geom = 'text', y = 3100, x = 0, hjust = 0, label=paste0("r = ",signif(Gcorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 2900, x = 0, hjust = 0, label=paste0("p = ",signif(Gcorr_all_p[3,1],digits = 2)), size=5)

s4 <- ggplot(aIM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("IPSL zmeso mgC m-2") + xlab("IPSL sst") + 
  annotate( geom = 'text', y = 1500, x = 2, hjust = 0, label=paste0("r = ",signif(Icorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 1400, x = 2, hjust = 0, label=paste0("p = ",signif(Icorr_all_p[3,1],digits = 2)), size=5)

s5 <- ggplot(aUM, aes(y=zoo, x=sst)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="blue", size = 0.75) +  
  ylab("UKESM zmeso mgC m-2") + xlab("UKESM sst") + 
  annotate( geom = 'text', y = 4500, x = 0, hjust = 0, label=paste0("r = ",signif(Ucorr_all_r[3,1],digits = 2)), size=5) +
  annotate( geom = 'text', y = 4000, x = 0, hjust = 0, label=paste0("p = ",signif(Ucorr_all_p[3,1],digits = 2)), size=5)

pdf( file = 'corr_ssp585_mesoz_sst_Stemp_FalWin_clim_200_log.pdf', width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
plot_grid( s1,s2,s3,s4,s5,
           nrow = 3, ncol = 2,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
write.table(Cff,"SSP585_coeffs_mod_chl_Stemp_FalWin_200.csv",sep=",",row.names=T)
...
Sff[5,] <- t(coefficients(UlmS))
row.names(Sff) <- c("CAN","CNRM","GFDL","IPSL","UKESM")
write.table(Sff,"SSP858_coeffs_mod_sst_Stemp_FalWin_200.csv",sep=",",row.names=T)
