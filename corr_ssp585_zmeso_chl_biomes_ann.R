...
# load data
CM <- read.csv("can_ssp585_zmeso200_chl_sst_clim_2051_2100_biomes.csv",sep=",",header = T,stringsAsFactors = F)
NM <- read.csv("cnrm_ssp585_zmeso200_chl_sst_clim_2051_2100_biomes.csv",sep=",",header = T,stringsAsFactors = F)
GM <- read.csv("gfdl_ssp585_zmeso200_chl_sst_clim_2051_2100_biomes.csv",sep=",",header = T,stringsAsFactors = F)
IM <- read.csv("ipsl_ssp585_zmeso200_chl_sst_clim_2051_2100_biomes.csv",sep=",",header = T,stringsAsFactors = F)
UM <- read.csv("ukesm_ssp585_zmeso200_chl_sst_clim_2051_2100_biomes.csv",sep=",",header = T,stringsAsFactors = F)
...
### Correlations -------------------------------------------------------------------
## Jessica said log-log for zoo-chl
...
pdf( file = paste0(figp,'corr_ssp585_mesoz_chl_biomes_all_log.pdf'), width = unit( 12, 'cm' ), height = unit( 10, 'cm' ) )
...
b1 <- ggplot(subset(CM, biomes==1), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  ...
  ylab("CAN zmeso gC m-2") + xlab("CAN chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 1e1, x = 1e-6, hjust = 0, label=paste0("r = ",signif(Ccorr[1,2],digits = 2)), size=5) 

b2 <- ggplot(subset(NM, biomes==1), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="red", size = 0.75) +
  ylab("CNRM zmeso gC m-2") + xlab("CNRM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 2, x = 5e-5, hjust = 0, label=paste0("r = ",signif(Ncorr[1,2],digits = 2)), size=5) 

b3 <- ggplot(subset(GM, biomes==1), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="red", size = 0.75) + 
  ylab("GFDL zmeso gC m-2") + xlab("GFDL chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 2, x = 3e-5, hjust = 0, label=paste0("r = ",signif(Gcorr[1,2],digits = 2)), size=5) 

b4 <- ggplot(subset(IM, biomes==1), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="red", size = 0.75) + 
  ylab("IPSL zmeso gC m-2") + xlab("IPSL chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 1, x = 5e-5, hjust = 0, label=paste0("r = ",signif(Icorr[1,2],digits = 2)), size=5) 
...
pdf( file = paste0(figp,'corr_ssp585_mesoz_chl_biome1_LC_log.pdf'), width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
c2 <- ggplot(subset(NM, biomes==2), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  ...
  ylab("CNRM zmeso gC m-2") + xlab("CNRM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 1, x = 1e-4, hjust = 0, label=paste0("r = ",signif(Ncorr[2,2],digits = 2)), size=5) 

c3 <- ggplot(subset(GM, biomes==2), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="green4", size = 0.75) + 
  ylab("GFDL zmeso gC m-2") + xlab("GFDL chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 3, x = 2e-4, hjust = 0, label=paste0("r = ",signif(Gcorr[2,2],digits = 2)), size=5) 

c4 <- ggplot(subset(IM, biomes==2), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="green4", size = 0.75) + 
  ylab("IPSL zmeso gC m-2") + xlab("IPSL chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 1, x = 1e-4, hjust = 0, label=paste0("r = ",signif(Icorr[2,2],digits = 2)), size=5) 

c5 <- ggplot(subset(UM, biomes==2), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm", se=FALSE, col="green4", size = 0.75) +
  ylab("UKESM zmeso gC m-2") + xlab("UKESM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 3, x = 1e-4, hjust = 0, label=paste0("r = ",signif(Ucorr[2,2],digits = 2)), size=5) 

pdf( file = paste0(figp,'corr_ssp585_mesoz_chl_biome2_HCSS_log.pdf'), width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
d2 <- ggplot(subset(NM, biomes==3), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  ...
  ylab("CNRM zmeso gC m-2") + xlab("CNRM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 4e0, x = 1e-4, hjust = 0, label=paste0("r = ",signif(Ncorr[3,2],digits = 2)), size=5) 
...
d5 <- ggplot(subset(UM, biomes==3), aes(y=zmeso, x=chl)) + theme_bw(base_size=14) +  
  ...
  ylab("UKESM zmeso gC m-2") + xlab("UKESM chl g m-3") + 
  scale_y_log10() + scale_x_log10() + 
  annotate( geom = 'text', y = 4e0, x = 1e-4, hjust = 0, label=paste0("r = ",signif(Ucorr[3,2],digits = 2)), size=5) 

pdf( file = paste0(figp,'corr_ssp585_mesoz_chl_biome3_HCPS_log.pdf'), width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
write.table(Cff1,"SSP585_coeffs_mod_chl_biome1_LC.csv",sep=",",row.names=T)
...
write.table(Cff2,"SSP585_coeffs_mod_chl_biome2_HCSS.csv",sep=",",row.names=T)
...
write.table(Cff3,"SSP585_coeffs_mod_chl_biome3_HCPS.csv",sep=",",row.names=T)
