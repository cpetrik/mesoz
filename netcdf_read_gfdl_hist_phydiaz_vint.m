...
%% phyc zall
ncdisp([fpath 'gfdl-esm4_r1i1p1f1_historical_phydiaz_vint_onedeg_global_monthly_1850_2014.nc'])

%%
% Size:       360x180x1980
standard_name = 'mole_concentration_of_diazotrophs_expressed_as_carbon_in_sea_water';
long_name     = 'Diazotroph Carbon Content';
units         = 'mol m-2';
...
%%
ncid = netcdf.open([fpath 'gfdl-esm4_r1i1p1f1_historical_phydiaz_vint_onedeg_global_monthly_1850_2014.nc'],'NC_NOWRITE');
...
for i = (nvars)
    varname = netcdf.inqVar(ncid, i-1);
    phydiazvint = netcdf.getVar(ncid,i-1);
    %eval([ varname '(' varname ' == 1.000000020040877e+20) = NaN;']);
end
netcdf.close(ncid);
...
%% mean over time
phydiaz_vint = squeeze(phydiazvint(:,:,runs));

%%
save([fpath 'gfdl_hist_phydiaz_vint_monthly_1951_2014.mat'],'phydiaz_vint','yr',...
