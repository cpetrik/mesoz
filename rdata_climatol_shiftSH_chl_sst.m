...
mod_chl(:,2) = (nchl_all(:)) * 1e3;
mod_chl(:,3) = (gchl_all(:)) * 1e6;
mod_chl(:,4) = (ichl_all(:)) * 1e3;
mod_chl(:,5) = (uchl_all(:)) * 1e6;
mod_chl(:,6) = (ochl_all(:));
...
writetable(tchl,'chl_model_obsglm_all_clim.csv')

tsst = array2table(matt,'VariableNames',...
    {'Lat','Lon','CAN','CNRM','GFDL','IPSL','UK','obsGLM'});
writetable(tsst,'sst_model_obsglm_all_clim.csv')

%%
save('chl_sst_model_obsglm_climatols.mat','matc','matt')
