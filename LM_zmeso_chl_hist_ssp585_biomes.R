...
hzoo1 <- matrix(NA, nrow = 5, ncol = 20)
hzoo2 <- matrix(NA, nrow = 5, ncol = 20)
hzoo3 <- matrix(NA, nrow = 5, ncol = 20)
#for(i in 1:6)
for(i in 1:5)
{
...
names(hzoo1) <- c("CAN","CNRM","GFDL","IPSL","UKESM")
names(hzoo2) <- c("CAN","CNRM","GFDL","IPSL","UKESM")
names(hzoo3) <- c("CAN","CNRM","GFDL","IPSL","UKESM")
...
pdf(file = paste0(figp,'Hist_SSP585_lm_zm_chl_biome1_LC.pdf'), width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
plot_grid( h1,s1,
           nrow = 1, ncol = 2,
...
h2 <- ggplot(Hdf2, aes(x = chl, y = value)) + 
  geom_line(aes(color = model)) + 
  scale_x_log10() + ggtitle("HCSS Hist") + 
  ylab("zmeso mgC m-2") + xlab("chl g m-3") + 
  scale_color_manual(values = c("green4","blue","purple","red","maroon"))

s2 <- ggplot(Sdf2, aes(x = chl, y = value)) + 
  geom_line(aes(color = model)) + 
  scale_x_log10() + ggtitle("HCSS SSP 585") + 
  ylab("zmeso mgC m-2") + xlab("chl g m-3") + 
  scale_color_manual(values = c("green4","blue","purple","red","maroon"))
...
h3 <- ggplot(Hdf3, aes(x = chl, y = value)) + 
  geom_line(aes(color = model)) + 
  scale_x_log10() + ggtitle("HCPS Hist") + 
  ylab("zmeso mgC m-2") + xlab("chl g m-3") + 
  scale_color_manual(values = c("green4","blue","purple","red","maroon"))

s3 <- ggplot(Sdf3, aes(x = chl, y = value)) + 
  geom_line(aes(color = model)) + 
  scale_x_log10() + ggtitle("HCPS SSP 585") + 
  ylab("zmeso mgC m-2") + xlab("chl g m-3") + 
  scale_color_manual(values = c("green4","blue","purple","red","maroon"))
...
# mdl2 <- lm(SSP$Lchl[2:5] ~ Hist$Lchl[2:5])
# ypred2 <- predict(mdl2)
# summary(mdl2)
