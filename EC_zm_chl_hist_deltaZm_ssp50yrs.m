...
%% Delta zmeso
load('delta_means_areaw_hist_ssp585_50yr_zmeso200_sspbiomes.mat');

%put on log10 scale
abs_diff = abs(zmeans_diff); %[1,3] is pos, all others neg
log_diff = log10(abs_diff);
log_diff = -1*log_diff;
log_diff(1,3) = -1*log_diff(1,3);
...
axis([0 2 -2.5 0.5])
ylabel('log_1_0 (\Delta zmeso)')
title('Future change')
text(0.1,1.25,'B','FontWeight','Bold','FontSize',14)
...
print('-dpng',[figp 'Hist_lm_SSP585_delta_EC_global_histbiomes.png'])
...
print('-dpng',[figp 'Hist_lm_SSP585_delta_EC_global_histbiomes_noCAN.png'])
