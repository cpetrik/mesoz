...
p1 <- ggplot(Rdf1, aes(obsGLM, mesoz)) + 
  ...
  theme_bw() + xlab("obs-GLMM") + ylab("raw mesozooplankton")

p2 <- ggplot(Ndf1, aes(obsGLM, mesoz)) + 
  geom_point() + facet_wrap(. ~ model, nrow=3, ncol=2) + 
  stat_smooth(method = "lm") +
  theme_bw() + xlab("obs-GLMM") + ylab("norm mesozooplankton")

p3 <- ggplot(Adf1, aes(obsGLM, mesoz)) + 
  geom_point() + facet_wrap(. ~ model, nrow=3, ncol=2) + 
  stat_smooth(method = "lm") +
  theme_bw() + xlab("obs-GLMM") + ylab("anom mesozooplankton")

p4 <- ggplot(Zdf1, aes(obsGLM, mesoz)) + 
  geom_point() + facet_wrap(. ~ model, nrow=3, ncol=2) + 
  stat_smooth(method = "lm") +
  theme_bw() + xlab("obs-GLMM") + ylab("0-1 mesozooplankton")

p5 <- ggplot(Odf1, aes(obsGLM, mesoz)) + 
  geom_point() + facet_wrap(. ~ model, nrow=3, ncol=2) + 
  stat_smooth(method = "lm") +
  theme_bw() + xlab("obs-GLMM") + ylab("-1-1 mesozooplankton")

pdf( file = paste0(figp,'Scatter_wrap_obsglm_all_clim_200_raw.pdf')) #, width = unit( 5, 'cm' ), height = unit( 5, 'cm' ) )
p1
dev.off()
pdf( file = paste0(figp,'Scatter_wrap_obsglm_all_clim_200_norm.pdf')) #, width = unit( 5, 'cm' ), height = unit( 5, 'cm' ) )
p2
dev.off()
pdf( file = paste0(figp,'Scatter_wrap_obsglm_all_clim_200_anom.pdf')) #, width = unit( 5, 'cm' ), height = unit( 5, 'cm' ) )
p3
dev.off()
pdf( file = paste0(figp,'Scatter_wrap_obsglm_all_clim_200_01.pdf')) #, width = unit( 5, 'cm' ), height = unit( 5, 'cm' ) )
p4
dev.off()
pdf( file = paste0(figp,'Scatter_wrap_obsglm_all_clim_200_neg11.pdf')) #, width = unit( 5, 'cm' ), height = unit( 5, 'cm' ) )
p5
dev.off()