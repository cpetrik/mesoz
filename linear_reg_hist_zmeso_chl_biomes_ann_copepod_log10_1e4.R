...
SM$Lzmeso <- log10(SM$zmeso200)#not needed +1e-4)
MM$Lzmeso <- log10(MM$zmeso200)#not needed +1e-4)

SM$Lchl <- log10(SM$chl) #+1e-16) #not needed min=0.03054
MM$Lchl <- log10(MM$chl) #+1e-16) #not needed min=0.03479
...
modelsummary(models, fmt=2) #estimate = "stars")
