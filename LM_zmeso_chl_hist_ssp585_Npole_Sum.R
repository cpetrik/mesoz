...
# Northern Polar regions, JJA climatology
# Historic and SSP585
...
# load data
Hist <- read.csv("Hist_coeffs_mod_chl_Npole_Sum_200.csv",sep=",",header = T,stringsAsFactors = F)
names(Hist) <- c("int","slop")

SSP <- read.csv("SSP585_coeffs_mod_chl_Npole_Sum_200.csv",sep=",",header = T,stringsAsFactors = F)
...
pdf(file = 'Hist_SSP585_lm_zm_chl_Npole_Sum_200.pdf', width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
plot_grid( b1,b2,
           nrow = 1, ncol = 2,
...
# pdf(file = 'Hist_SSP585_lm_zm_chl_Npole_Sum_200_noCAN.pdf', width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
# plot_grid( a1,a2,
#            nrow = 1, ncol = 2,
#            rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
#            align = 'h' )
...
summary(mdl)
#Multiple R-squared:  0.0187,	Adjusted R-squared:  -0.3084 
#F-statistic: 0.05718 on 1 and 3 DF,  p-value: 0.8264

mdl2 <- lm(SSP$slop[2:5] ~ Hist$slop[2:5])
ypred2 <- predict(mdl2)
summary(mdl2)
#Multiple R-squared:  0.7841,	Adjusted R-squared:  0.6761 
#F-statistic: 7.263 on 1 and 2 DF,  p-value: 0.1145
...
c1 <- ggplot(EC, aes(x = Hist, y = SSP)) + 
  ...
  annotate( geom = 'text', y = 4.5, x = -1, hjust = 0, label="r = -0.31", size=5) +
  annotate( geom = 'text', y = 4, x = -1, hjust = 0, label="p = 0.83", size=5)
...
c2 <- ggplot(EC2, aes(x = Hist, y = SSP)) + 
  ...
  annotate( geom = 'text', y = 2, x = 0.3, hjust = 0, label="r = 0.68", size=5) +
  annotate( geom = 'text', y = 1.8, x = 0.3, hjust = 0, label="p = 0.11", size=5)


pdf(file = 'Hist_SSP585_lm_zm_chl_EC_Npole_Sum_200.pdf', width = unit( 15, 'cm' ), height = unit( 4.5, 'cm' ))
plot_grid( b1,b2,c1,
           nrow = 1, ncol = 3,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
pdf(file = 'Hist_SSP585_lm_zm_chl_EC_Npole_Sum_200_noCAN.pdf', width = unit( 15, 'cm' ), height = unit( 4.5, 'cm' ))
plot_grid( a1,a2,c2,
           nrow = 1, ncol = 3,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
# load data
Hist <- read.csv("Hist_coeffs_mod_sst_Npole_Sum_200.csv",sep=",",header = T,stringsAsFactors = F)
names(Hist) <- c("int","slop")

SSP <- read.csv("SSP585_coeffs_mod_sst_Npole_Sum_200.csv",sep=",",header = T,stringsAsFactors = F)
names(SSP) <- c("int","slop")
...
pdf(file = 'Hist_SSP585_lm_zm_sst_Npole_Sum_200.pdf', width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
plot_grid( b1,b2,
           nrow = 1, ncol = 2,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
ypred <- predict.lm(mdl)
summary(mdl)
#Multiple R-squared:  0.9811,	Adjusted R-squared:  0.9748 
#F-statistic: 155.6 on 1 and 3 DF,  p-value: 0.001111
...
c1 <- ggplot(EC, aes(x = Hist, y = SSP)) + 
  ...
  scale_color_manual(values = c("green4","blue","purple","red","maroon")) +
  geom_line(aes(x = Hist, y = ypred)) + 
  annotate( geom = 'text', y = 150, x = 0, hjust = 0, label="r = 0.97", size=5) +
  annotate( geom = 'text', y = 140, x = 0, hjust = 0, label="p = 0.001", size=5)


pdf(file = 'Hist_SSP585_lm_zm_sst_EC_Npole_Sum_200.pdf', width = unit( 15, 'cm' ), height = unit( 4.5, 'cm' ))
plot_grid( b1,b2,c1,
           nrow = 1, ncol = 3,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
