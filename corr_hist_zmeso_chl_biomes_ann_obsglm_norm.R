...
## OBS CHL and SST w/BIOMES
OM <- read.csv("glm_obs_zmeso_chl_sst_ann_mean_biomes.csv",sep=",",header = T,stringsAsFactors = F)
OM <- na.omit(OM)
...
## Normalize zmeso 
CM$Szmeso <- scale(CM$Lzmeso)
NM$Szmeso <- scale(NM$Lzmeso)
GM$Szmeso <- scale(GM$Lzmeso)
IM$Szmeso <- scale(IM$Lzmeso)
UM$Szmeso <- scale(UM$Lzmeso)
OM$Szmeso <- scale(OM$Lzmeso)
...
Ccorr <- ddply(CM, .(biomes), summarize, "corr" = cor(Szmeso, Lchl, method = "pearson"))
Ncorr <- ddply(NM, .(biomes), summarize, "corr" = cor(Szmeso, Lchl, method = "pearson"))
Gcorr <- ddply(GM, .(biomes), summarize, "corr" = cor(Szmeso, Lchl, method = "pearson"))
Icorr <- ddply(IM, .(biomes), summarize, "corr" = cor(Szmeso, Lchl, method = "pearson"))
Ucorr <- ddply(UM, .(biomes), summarize, "corr" = cor(Szmeso, Lchl, method = "pearson"))
Ocorr <- ddply(OM, .(biomes), summarize, "corr" = cor(Szmeso, Lchl, method = "pearson"))
...
a1 <- ggplot(CM, aes(y=zmeso, x=chl, color=factor(biomes))) + theme_bw(base_size=14) +  
  ...
  ylab("CAN norm zmeso") + xlab("CAN chl g m-3") + 
...
a2 <- ggplot(NM, aes(y=zmeso, x=chl, color=factor(biomes))) + theme_bw(base_size=14) +  
  ...
  ylab("CNRM norm zmeso") + xlab("CNRM chl g m-3") + 
  scale_y_log10() + scale_x_log10() 
  
a3 <- ggplot(GM, aes(y=zmeso, x=chl, color=factor(biomes))) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm") +  
  ylab("GFDL norm zmeso") + xlab("GFDL chl g m-3") + 
  scale_y_log10() + scale_x_log10() 
  
a4 <- ggplot(IM, aes(y=zmeso, x=chl, color=factor(biomes))) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm") +  
  ylab("IPSL norm zmeso") + xlab("IPSL chl g m-3") + 
  scale_y_log10() + scale_x_log10() 
  
a5 <- ggplot(UM, aes(y=zmeso, x=chl, color=factor(biomes))) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm") +
  ylab("UKESM norm zmeso") + xlab("UKESM chl g m-3") + 
  scale_y_log10() + scale_x_log10() 
  
a6 <- ggplot(OM, aes(y=zmeso, x=chl, color=factor(biomes))) + theme_bw(base_size=14) +  
  geom_point() + geom_smooth(method="lm") +  
  ylab("Obs norm zmeso") + xlab("Obs chl g m-3") + 
  scale_y_log10() + scale_x_log10() 

pdf( file = paste0(figp,'corr_hist_mesoz_chl_biomes_all_log_obsglm.pdf'), width = unit( 12, 'cm' ), height = unit( 10, 'cm' ) )
...
pdf( file = paste0(figp,'corr_hist_mesoz_chl_biome1_LC_log_obsglm.pdf'), width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
pdf( file = paste0(figp,'corr_hist_mesoz_chl_biome2_HCSS_log_obsglm.pdf'), width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
pdf( file = paste0(figp,'corr_hist_mesoz_chl_biome3_HCPS_log_obsglm.pdf'), width = unit( 10, 'cm' ), height = unit( 12, 'cm' ) )
...
#CAN
Clm1 <- lm(Szmeso~Lchl, data=subset(CM, biomes==1))
Clm2 <- lm(Szmeso~Lchl, data=subset(CM, biomes==2))
Clm3 <- lm(Szmeso~Lchl, data=subset(CM, biomes==3))
summary(Clm1)
summary(Clm2)
summary(Clm3)

#CNRM
Nlm1 <- lm(Szmeso~Lchl, data=subset(NM, biomes==1))
Nlm2 <- lm(Szmeso~Lchl, data=subset(NM, biomes==2))
Nlm3 <- lm(Szmeso~Lchl, data=subset(NM, biomes==3))
summary(Nlm1)
summary(Nlm2)
summary(Nlm3)

#GFDL
Glm1 <- lm(Szmeso~Lchl, data=subset(GM, biomes==1))
Glm2 <- lm(Szmeso~Lchl, data=subset(GM, biomes==2))
Glm3 <- lm(Szmeso~Lchl, data=subset(GM, biomes==3))
summary(Glm1)
summary(Glm2)
summary(Glm3)

#IPSL
Ilm1 <- lm(Szmeso~Lchl, data=subset(IM, biomes==1))
Ilm2 <- lm(Szmeso~Lchl, data=subset(IM, biomes==2))
Ilm3 <- lm(Szmeso~Lchl, data=subset(IM, biomes==3))
summary(Ilm1)
summary(Ilm2)
summary(Ilm3)

#UKESM
Ulm1 <- lm(Szmeso~Lchl, data=subset(UM, biomes==1))
Ulm2 <- lm(Szmeso~Lchl, data=subset(UM, biomes==2))
Ulm3 <- lm(Szmeso~Lchl, data=subset(UM, biomes==3))
summary(Ulm1)
summary(Ulm2)
summary(Ulm3)

#Obs
Olm1 <- lm(Szmeso~Lchl, data=subset(OM, biomes==1))
Olm2 <- lm(Szmeso~Lchl, data=subset(OM, biomes==2))
Olm3 <- lm(Szmeso~Lchl, data=subset(OM, biomes==3))
summary(Olm1)
summary(Olm2)
summary(Olm3)
...
write.table(Cff1,"Hist_coeffs_mod_chl_biome1_LC_obsglm_norm.csv",sep=",",row.names=T)
...
write.table(Cff2,"Hist_coeffs_mod_chl_biome2_HCSS_obsglm_norm.csv",sep=",",row.names=T)
...
write.table(Cff3,"Hist_coeffs_mod_chl_biome3_HCPS_obsglm_norm.csv",sep=",",row.names=T)
...
write.table(Oerr1,"Hist_stderr_predict_obsglm_norm_chl_biome1.csv",sep=",",row.names=T)
write.table(Oerr2,"Hist_stderr_predict_obsglm_norm_chl_biome2.csv",sep=",",row.names=T)
write.table(Oerr3,"Hist_stderr_predict_obsglm_norm_chl_biome3.csv",sep=",",row.names=T)
...
write.table(Er,"Hist_stderr_coeff_obsglm_norm_chl_all_biomes.csv",sep=",",row.names=T)
