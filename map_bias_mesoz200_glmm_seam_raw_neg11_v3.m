...
%% Log-trans
cmin = 0.5 * min(cz_s(cz_s(:) > 0));
mmin = 0.5 * min(mz_s(mz_s(:) > 0));
nmin = 0.5 * min(nz_s(nz_s(:) > 0));
gmin = 0.5 * min(gz_s(gz_s(:) > 0));
imin = 0.5 * min(iz_s(iz_s(:) > 0));
umin = 0.5 * min(uz_s(uz_s(:) > 0));
omin = 0.5 * min(oz_o(oz_o(:) > 0));
...
stamp(stex)
print('-dpng',[ppath 'Map_all_hist_clim_glmm_log_scale_v3.png'])
...
text(0.2,1.65,'UKESM','HorizontalAlignment','center','FontWeight','bold')
stamp(stex)
print('-dpng',[ppath 'Map_all_hist_clim_glmm_neg11_scale_v3.png'])
...
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
stamp(stex)
print('-dpng',[ppath 'Map_bias_all_hist_clim_glmm_log_scale_v3.png'])
...
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
stamp(stex)
print('-dpng',[ppath 'Map_bias_all_hist_clim_glmm_neg11_scale_v3.png'])
