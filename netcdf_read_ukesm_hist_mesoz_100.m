...
for n = nvars
    varname = netcdf.inqVar(ncid, n-1);
    zmeso = netcdf.getVar(ncid,n-1, [0,0,0,runs1(1)-1],[360 330 length(z100) length(runs1)]);
...
%yr = ((time+1)/12)+1850;
yr2 = ((time+1)/360)+1850;
runs2 = find(yr2>1950 & yr2<=2015);
z100 = find(lev <= 100);

for n = nvars
    varname = netcdf.inqVar(ncid, n-1);
    zmeso = netcdf.getVar(ncid,n-1, [0,0,0,runs2(1)-1],[360 330 length(z100) length(runs2)]);
    zmeso(zmeso >= 1.00e+20) = NaN;
end
netcdf.close(ncid);
...
%%
save([fpath 'ukesm_hist_zmeso100_monthly_1950_2014.mat'],'zmeso_100','yr1',...
    'yr2','runs1','runs2','long_name','standard_name','units','z100',...
    'lev','lev_bnds','latitude','longitude');
