...
%% sst
ncdisp([bpath 'UKESM_historical_biomes_x1.nc'])
...
% biomes:chlorophyll-threshold = "0.140 mg Chl m^-3" ;


%% Hist
ncid = netcdf.open([bpath 'UKESM_historical_biomes_x1.nc'],'NC_NOWRITE');
...
%%
save([bpath 'UKESM_historical_biomes.mat'],'lat','lon','biomes','hubiomes');

%% SSP585
clear lat lon biomes

ncid = netcdf.open([bpath 'UKESM_ssp585_biomes_x1.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
...
%%
save([bpath 'UKESM_ssp585_biomes.mat'],'lat','lon','biomes','subiomes');
...
axesm ('Robinson','MapLatLimit',clatlim,'MapLonLimit',clonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
surfm(lat_g,lon_g,subiomes)
colormap(cmv)
%caxis([1 3])
title('UK SSP585')
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);

print('-dpng',[ppath 'Map_UK_biomes_hist_ssp585.png'])
