...
#obsMO from mgC to gC
MOS$zmeso200 <- MOS$zmeso200*1e-3
MOM$zmeso200 <- MOM$zmeso200*1e-3

#log10 trans
CM$Lzmeso <- log10(CM$mesoz+1e-4)
MM$Lzmeso <- log10(MM$mesoz+1e-4)
NM$Lzmeso <- log10(NM$mesoz+1e-4)
GM$Lzmeso <- log10(GM$mesoz+1e-4)
IM$Lzmeso <- log10(IM$mesoz+1e-4)
UM$Lzmeso <- log10(UM$mesoz+1e-4)
OG$Lzmeso <- log10(OG$mesoz+1e-4)
OS$Lzmeso <- log10(OS$mesoz+1e-4)
MOS$Lzmeso <- log10(MOS$zmeso200 +1e-4)
MOM$Lzmeso <- log10(MOM$zmeso200 +1e-4)

CM$Lchl <- log10(CM$chl+1e-4)
MM$Lchl <- log10(MM$chl+1e-4)
NM$Lchl <- log10(NM$chl+1e-4)
GM$Lchl <- log10(GM$chl+1e-4)
IM$Lchl <- log10(IM$chl+1e-4)
UM$Lchl <- log10(UM$chl+1e-4)
OG$Lchl <- log10(OG$chl+1e-4)
OS$Lchl <- log10(OS$chl+1e-4)
MOS$Lchl <- log10(MOS$chl+1e-4) 
MOM$Lchl <- log10(MOM$chl+1e-4) 
...
write.table(Cff0,paste0(datap,"Hist_coeffs_mod_chl_global_obsglm100_strom_cope.csv"),sep=",",row.names=T)
...
write.table(Cff1,paste0(datap,"Hist_coeffs_mod_chl_biome1_LC45_obsglm100_strom_cope.csv"),sep=",",row.names=T)
...
write.table(Cff2,paste0(datap,"Hist_coeffs_mod_chl_biome2_HCSS_obsglm100_strom_cope.csv"),sep=",",row.names=T)
...
write.table(Cff3,paste0(datap,"Hist_coeffs_mod_chl_biome3_HCPS_obsglm100_strom_cope.csv"),sep=",",row.names=T)
...
modelsummary(Gmodels, fmt=2, output = paste0(datap,"regress_global_table.docx"))
...
modelsummary(Lmodels, fmt=2, output = paste0(datap,"regress_LC_table.docx"))
...
modelsummary(Smodels, fmt=2, output = paste0(datap,"regress_HCSS_table.docx"))
...
modelsummary(Pmodels, fmt=2, output = paste0(datap,"regress_HCPS_table.docx"))
