...
%%
load('Hist_SSP585_coeffs_chl_global_biomes.mat');
...
x = (-5:0.5:4);
xs = 0:0.5:6;
y1 = Hist.slop(7,1)*ones(size(xs));
y2 = Hist.slop(7,2)*ones(size(xs));
...
print('-dpng',[figp 'Hist_SSP585_lm_EC_zmeso_chl_biomes_LC45_cmcc.png'])
...
print('-dpng',[figp 'Hist_SSP585_lm_EC_zmeso_chl_biomes_LC45_cmcc_noCAN.png'])
...
plot(log(chl),hzoo4(6,:),'color',cb(7,:),'LineWidth',2); hold on;
% legend(hmod)
% legend('location','northwest')
ylim([-3 2])
xlim([round(cmin) round(cmax)])
...
plot(log(chl),hzoo1(6,:),'color',cb(7,:),'LineWidth',2); hold on;
% legend(hmod)
% legend('location','northwest')
ylim([-5 6])
xlim([round(cmin) round(cmax)])
...
plot(log(chl),hzoo2(6,:),'color',cb(7,:),'LineWidth',2); hold on;
ylim([-3 3])
xlim([round(cmin) round(cmax)])
% xlabel('log chl (mg m^-^3)')
ylabel('HCSS')
...
plot(log(chl),hzoo3(6,:),'color',cb(7,:),'LineWidth',2); hold on;
ylim([-3 2])
xlim([round(cmin) round(cmax)])
xlabel('log chl (mg m^-^3)')
ylabel('HCPS')
