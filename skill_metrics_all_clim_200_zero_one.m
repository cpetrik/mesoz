...
Amax = nanmax(Acomb);
Amin  = nanmin(Acomb);
Acomb = (Acomb - Amin) ./ (Amax - Amin) ;

Dmax = nanmax(Dcomb);
Dmin  = nanmin(Dcomb);
Dcomb = (Dcomb - Dmin) ./ (Dmax - Dmin) ;

Jmax = nanmax(Jcomb);
Jmin  = nanmin(Jcomb);
Jcomb = (Jcomb - Jmin) ./ (Jmax - Jmin) ;

Mmax = nanmax(Mcomb);
Mmin  = nanmin(Mcomb);
Mcomb = (Mcomb - Mmin) ./ (Mmax - Mmin) ;

Smax = nanmax(Scomb);
Smin  = nanmin(Scomb);
Scomb = (Scomb - Smin) ./ (Smax- Smin) ;
...
simtext = {'CAN','CNRM','GFDL','IPSL','UK'};
simtex = {'CA','CN','GF','IP','UK'};
...
set(gca,'XTickLabel',simtex)
%ylim([-15 5])
print('-dpng',[figp 'corr_rmse_bias_clims_200_obsglm_01_subplot.png'])

%%
ctext = {'All','Winter','Spring','Summer','Fall'};
save('skill_scores_model_obsglm_clim_01.mat','skill','simtext','metrics','ctext')
...
title('Bias')
set(gca,'XTickLabel',ctext)
print('-dpng',[figp 'corr_rmse_bias_clims_200_obsglm_01_together.png'])
...
writetable(Tcorr,'corr_clims_200_obsglm_01.csv','WriteRowNames',true);
writetable(Trmse,'rmse_clims_200_obsglm_01.csv','WriteRowNames',true);
writetable(Tnstd,'nstd_clims_200_obsglm_01.csv','WriteRowNames',true);
writetable(Turmse,'urmse_clims_200_obsglm_01.csv','WriteRowNames',true);
writetable(Ttrmse,'trmse_clims_200_obsglm_01.csv','WriteRowNames',true);
writetable(Tbias,'bias_clims_200_obsglm_01.csv','WriteRowNames',true);
