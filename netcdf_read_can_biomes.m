...
%% 
ncdisp([bpath 'CanESM_historical_biomes_x1.nc'])
...
% biomes:chlorophyll-threshold = "0.255 mg Chl m^-3" ;


%% Hist
ncid = netcdf.open([bpath 'CanESM_historical_biomes_x1.nc'],'NC_NOWRITE');
...
%%
save([bpath 'CanESM_historical_biomes.mat'],'lat','lon','biomes','hcbiomes');

%% SSP585
clear lat lon biomes

ncid = netcdf.open([bpath 'CanESM_ssp585_biomes_x1.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
...
%%
save([bpath 'CanESM_ssp585_biomes.mat'],'lat','lon','biomes','scbiomes');
...
axesm ('Robinson','MapLatLimit',clatlim,'MapLonLimit',clonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
surfm(lat_g,lon_g,scbiomes)
colormap(cmv)
%caxis([1 3])
title('CAN SSP585')
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);

print('-dpng',[ppath 'Map_CAN_biomes_hist_ssp585.png'])
