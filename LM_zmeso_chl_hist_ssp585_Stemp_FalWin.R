...
# load data
Hist <- read.csv("Hist_coeffs_mod_chl_Stemp_FalWin_200.csv",sep=",",header = T,stringsAsFactors = F)
names(Hist) <- c("int","slop")

SSP <- read.csv("SSP585_coeffs_mod_chl_Stemp_FalWin_200.csv",sep=",",header = T,stringsAsFactors = F)
...
pdf(file = 'Hist_SSP585_lm_zm_chl_Stemp_FalWin_200.pdf', width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
plot_grid( b1,b2,
           nrow = 1, ncol = 2,
...
pdf(file = 'Hist_SSP585_lm_zm_chl_Stemp_FalWin_200_noCAN.pdf', width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
plot_grid( a1,a2,
           nrow = 1, ncol = 2,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
#F-statistic: 4.491e+04 on 1 and 3 DF,  p-value: 2.317e-07

mdl2 <- lm(SSP$slop[2:5] ~ Hist$slop[2:5])
ypred2 <- predict(mdl2)
summary(mdl2)
#Multiple R-squared:  0.9793,	Adjusted R-squared:  0.969 
#F-statistic: 94.72 on 1 and 2 DF,  p-value: 0.01039
...
c1 <- ggplot(EC, aes(x = Hist, y = SSP)) + 
  ...
  annotate( geom = 'text', y = 8, x = 1, hjust = 0, label="r = 0.99", size=5) +
  annotate( geom = 'text', y = 7.5, x = 1, hjust = 0, label="p = 2.3e-7", size=5)
...
c2 <- ggplot(EC2, aes(x = Hist, y = SSP)) + 
  ...
  annotate( geom = 'text', y = 1, x = 0.4, hjust = 0, label="r = 0.97", size=5) +
  annotate( geom = 'text', y = 0.95, x = 0.4, hjust = 0, label="p = 0.01", size=5)


pdf(file = 'Hist_SSP585_lm_zm_chl_EC_Stemp_FalWin_clim_200.pdf', width = unit( 15, 'cm' ), height = unit( 4.5, 'cm' ))
plot_grid( b1,b2,c1,
           nrow = 1, ncol = 3,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
pdf(file = 'Hist_SSP585_lm_zm_chl_EC_Stemp_FalWin_200_noCAN.pdf', width = unit( 15, 'cm' ), height = unit( 4.5, 'cm' ))
plot_grid( a1,a2,c2,
           nrow = 1, ncol = 3,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
# load data
Hist <- read.csv("Hist_coeffs_mod_sst_Stemp_FalWin_200.csv",sep=",",header = T,stringsAsFactors = F)
names(Hist) <- c("int","slop")

SSP <- read.csv("SSP585_coeffs_mod_sst_Stemp_FalWin_200.csv",sep=",",header = T,stringsAsFactors = F)
names(SSP) <- c("int","slop")
...
pdf(file = 'Hist_SSP585_lm_zm_sst_Stemp_FalWin_200.pdf', width = unit( 10, 'cm' ), height = unit( 5, 'cm' ))
plot_grid( b1,b2,
           nrow = 1, ncol = 2,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
...
ypred <- predict.lm(mdl)
summary(mdl)
#Multiple R-squared:  0.4965,	Adjusted R-squared:  0.3286 
#F-statistic: 2.958 on 1 and 3 DF,  p-value: 0.1839
...
c1 <- ggplot(EC, aes(x = Hist, y = SSP)) + 
  ...
  scale_color_manual(values = c("green4","blue","purple","red","maroon")) +
  geom_line(aes(x = Hist, y = ypred)) + 
  annotate( geom = 'text', y = 30, x = 0, hjust = 0, label="r = 0.33", size=5) +
  annotate( geom = 'text', y = 25, x = 0, hjust = 0, label="p = 0.18", size=5)


pdf(file = 'Hist_SSP585_lm_zm_sst_EC_Stemp_FalWin_200.pdf', width = unit( 15, 'cm' ), height = unit( 4.5, 'cm' ))
plot_grid( b1,b2,c1,
           nrow = 1, ncol = 3,
           rel_widths = c( 1, 1 ), rel_heights = c( 1, 1 ) ,
           align = 'h' )
